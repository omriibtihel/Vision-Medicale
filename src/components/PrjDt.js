...
const ProjectDetail = () => {
  ...
  const handleFileChange = (e) => {
    ...
    if (isCSV) reader.readAsText(file);
      ...
      alert(
        "Seuls les fichiers CSV (.csv) ou Excel (.xls, .xlsx) sont acceptés"
      );
  };
  ...
  const saveDatabase = async (file, fileName, date) => {
    ...
    try {
      await axios.post(
        `http://localhost:5000/import-database/${id}`,
        formData,
        {
          headers: {
          ...
          <div className="project-hero-modern">
            ...
            <div className="project-info-modern">
              ...
              <p className="project-description-modern">
                Gestion des données médicales et analyses IA
              </p>
              ...
              {fileName && fileDetails ? (
                ...
                <div className="empty-state-modern">
                  <p className="empty-message-modern">
                    Aucun fichier n'a été importé pour ce projet
                  </p>
                </div>
              )}
            </div>
          ...
          {fileName ? (
            ...
            >
              ...
              <div className="dropzone-content-modern">
                ...
                />
                <h3 className="dropzone-title-modern">
                  Importer vos données médicales
                </h3>
                <p className="dropzone-text-modern">
                  Glissez-déposez un fichier CSV ou Excel ici, ou cliquez pour
                  sélectionner
                </p>
                  ...
                  {filePreview.map((row, idx) => (
                    <tr key={idx}>
                      {Object.values(row).map((cell, i) => (
                        <td key={i}>
                          {cell || <span className="empty-cell-modern">-</span>}
                        </td>
                      ))}
                    </tr>
                  ))}
