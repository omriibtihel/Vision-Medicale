...
const ImportSuccess = () => {
  ...
  const importBD = async () => {
    ...
    try {
      ...
      if (Array.isArray(cleanedData)) {
        ...
        setTotalColumns(
          cleanedData.length > 0 ? Object.keys(cleanedData[0]).length : 0
        );
    ...
    try {
      ...
      if (error.response) {
        if (error.response.status === 400) {
          setTargetFeatureError(
            error.response.data.error || "Invalid target feature"
          );
  ...
  const histogramData = {
    labels: histogramCounts.map(
      (_, i) =>
        `${(minVal + i * binSize).toFixed(2)} - ${(
          minVal +
          (i + 1) * binSize
        ).toFixed(2)}`
    ),
  ...
  const filteredCols =
    selectedFeatures.length > 0 ? selectedFeatures : numericCols;

  const heatmapData = filteredCols.map((rowCol) =>
    filteredCols.map((col) => {
      const rowValues = data
        .map((r) => parseFloat(r[rowCol]))
        .filter((v) => !isNaN(v));
      ...
      const cov =
        rowValues.reduce(
          (acc, v, i) => acc + (v - meanX) * (colValues[i] - meanY),
          0
        ) / n;
      const stdX = Math.sqrt(
        rowValues.reduce((acc, v) => acc + Math.pow(v - meanX, 2), 0) / n
      );
      const stdY = Math.sqrt(
        colValues.reduce((acc, v) => acc + Math.pow(v - meanY, 2), 0) / n
      );
  ...
  const filteredRows = heatmapData
    .map((row, i) => ({
      ...
      filteredRows
        .map((r) =>
          [
            r.rowName,
            ...r.values
              .filter((v) => visibleCols.includes(v.col))
              .map((v) => (v.show ? v.val : "")),
          ].join(",")
        )
        .join("\n");
  ...
  const getMedicalSuggestionsForFeature = (feature, values, type) => {
    ...
    if (values.some((v) => v === "null" || v === "" || v === null)) {
      suggestions.push(
        "🧹 Nettoyage : détecter les valeurs manquantes ou invalides."
      );
    }
    ...
    if (type === "number") {
      ...
      suggestions.push(
        "🧩 Imputation (médiane) : pour gérer les valeurs manquantes de manière robuste."
      );
      ...
      if (std > 0 && max !== min) {
        suggestions.push(
          "📏 Normalisation (Z-score) : si la distribution est normale."
        );
        suggestions.push(
          "📏 Normalisation (Min-Max) : si les bornes physiologiques sont connues."
        );
      }
    }
    ...
    if (type === "string") {
      suggestions.push(
        "🧩 Imputation (mode) : pour remplacer les valeurs manquantes par la plus fréquente."
      );
    }
    ...
    >
      ...
      >
        ...
        <div className="description">
          <p>
            <strong>Nombre total de lignes :</strong> {totalRows}
          </p>
          <p>
            <strong>Nombre total de colonnes :</strong> {totalColumns}
          </p>
            ...
            {!isTargetFeatureSet ? (
              <div className="target-feature-selector">
                ...
                <div className="target-feature-controls">
                  ...
                  >
                    <option value="">-- Selectionner la cible --</option>
                ...
                {targetFeatureError && (
                  <div className="error-message">{targetFeatureError}</div>
                )}
              </div>
            ) : (
              <div className="target-feature-display">
                <h3>Cible actuelle</h3>
        ...
        <div className="heatmap-section">
          ...
          >
            <label htmlFor="corr-threshold">Corrélation ≥</label>
            <input
              id="corr-threshold"
              type="range"
              min="0"
            ...
            {isFeaturePopupOpen && (
              ...
              >
                ...
                <div className="feature-buttons" style={{ marginTop: "1rem" }}>
                  <button onClick={() => setSelectedFeatures(numericCols)}>
                    ✅ Tout sélectionner
                  </button>
                    ...
                    {visibleCols.map((col, j) => {
                      ...
                      return (
                        <td
                          key={j}
                          style={{
                            background: cell?.show
                              ? `rgba(54, 162, 235, ${Math.abs(cell.val)})`
                              : "#f0f0f0",
                            color:
                              cell?.show && Math.abs(cell.val) > 0.5
                                ? "white"
                                : "black",
                            textAlign: "center",
                          }}
            ...
            <>
              ...
              <div className="missing-donut-container">
                ...
                <div className="missing-donut-percent">
                  {((missingCounts[selectedFeature] / totalRows) * 100).toFixed(
                    1
                  )}
                  %
                </div>
              ...
              {suggestions.length > 0 && (
                ...
                >
                  <h4 style={{ color: "#003366", marginBottom: "0.5rem" }}>
                    💡 Suggestions pour "{selectedFeature}"
                  </h4>
                  <ul style={{ paddingLeft: "1.2rem" }}>
                    {suggestions.map((sugg, idx) => (
                      <li
                        key={idx}
                        style={{ marginBottom: "0.4rem", color: "#333" }}
                      >
                        {sugg}
                      </li>
                    ))}
