...
import { 
  ...
  faDatabase, faFileAlt, faHistory, 
  faRocket, faChevronLeft, faChevronRight , faTimes
...
const Sidebar = ({ isOpen, toggleSidebar, projectId, targetFeature }) => {
  ...
  const handleGraphsClick = () => navigate(`/graphs/${projectId}/${targetFeature}`);
  const handleProcessingClick = () => navigate(`/processing/${projectId}/${targetFeature}`);
  const handleModelsClick = () => navigate(`/models/${projectId}/${targetFeature}`);
  const handleDBClick = () => navigate(`/importSucc/${projectId}`);
  const handleDescription = () => navigate(`/description/${projectId}/${targetFeature}`);
  const handleHistorique = () => navigate(`/historique/${projectId}/${targetFeature}`);
  const handleDepClick = () => navigate(`/deployment/${projectId}/${targetFeature}`);

  const navItems = [
    { icon: faUser, label: "Profile", action: handleProfileClick, active: false },
    { icon: faDatabase, label: "Database", action: handleDBClick, active: true },
    ...
    { icon: faCog, label: "Processing", action: handleProcessingClick, active: false },
